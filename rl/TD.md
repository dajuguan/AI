...
| **估计量方差** $\mathrm{Var}(\hat V(s))$ | 不同随机 episode(**跨 run**)**学出的 $\hat V$** 有多飘 | **后果** | 图三(跨 run 的 $\hat V(s_0)$ 误差棒);图一的 RMS 是它与偏差²的**合成** |
...
**图三:把偏差和方差拆到"估计量"这一侧——训练后的 $\hat V(s_0)$**

![训练后 V̂(s0) 的偏差与估计量方差随 λ 变化](imgs/estimator-bias-variance-vs0.svg)

图二拆的是"更新目标"$G_t^\lambda$ 的分布(**根源**);这张图把同一套偏差/方差搬到"学出来的估计量"这一侧(**后果**)。做法:**每次训练跑到收敛(300 个 episode、$\alpha=0.05$),用 50 批独立数据各训练一次**,只盯状态 $15$(真值 $+0.5$)的最终 $\hat V(s_0)$。(episode 数取够大是关键——不然 TD 还没学到真值,那个"偏差"只是欠训练的假象,而非算法本身的偏差。)

- **口径最关键(别踩坑):** 这里的方差是**跨 50 次独立 run** 的最终 $\hat V(s_0)$ 的散布 $\mathrm{Var}(\hat V(s_0))$,**不是**单次训练内 $\hat V(s_0)$ 逐 episode 的历史抖动——后者混进了"还没收敛"的暂态,不是分解式里的估计量方差。偏差同理是**跨 run 的最终值均值** 减真值,不是"历史均值减真值"。这正是本节前面第 369 行钉的:"换一批 episode 训练,就学出不同的 $\hat V$;这堆结果的散布,才是 $\mathrm{Var}(\hat V(s))$"。
- **(a)** 各 λ 的 $\hat V(s_0)$ 随 episode 收敛(均值线 ± 1 std 带):**λ 大的收敛快但带子宽又抖**($\lambda=1$ MC 几十个 episode 就摸到真值线,但带子最宽);**λ 小的收敛慢但带窄**($\lambda=0$ TD(0) 要三百个 episode 才慢慢爬到 $0.5$,但几乎不抖)。这就是资格迹"信息快速回传"与"高方差"两面的直观图像。
- **(b)** 训练收敛后跨 run 的最终 $\hat V(s_0)$:点=均值,棒长=std(= **估计量方差**)。**所有 λ 的点都落在真值 $0.5$ 附近(收敛后基本无偏)**,差异**纯粹是方差**——误差棒随 $\lambda$ **单调变长**(std 从 $\lambda{=}0$ 的 $0.04$ 涨到 $\lambda{=}1$ 的 $0.49$)。这正是**估计量侧对图二 (a) 的印证**:偏差被训练消掉后,剩下的就是"λ↑ → 方差↑"这条纯方差轴。
- 注:偏差在这里被"训练到收敛"抹平了(tabular TD(λ) 各 λ 都渐近无偏);图二 (b) 那条"偏差随 λ 降"的曲线,刻画的是**训练不足 / $V$ 不准**时的暂态偏差,与本图收敛后的情形互补,不矛盾。

> **三图合起来讲完整个故事:** 图二说明"目标侧" $\lambda$ 是 bias-variance 旋钮(方差随 $\lambda$ 升、偏差在自举依赖不准 $V$ 时随 $\lambda$ 降);图三说明这个根源如实**传导到估计量侧**——收敛后偏差归零,跨 run 学出的 $\hat V$ 仍随 $\lambda$ 越来越飘;图一则把偏差² 与方差**合成**成一个总误差(RMS),于是在**有限训练预算**下(偏差、方差都还在)才出现折中的 U 形 sweet spot。这把本文最前面 MC/TD 的定位、中间的 $n$-step / TD($\lambda$) 旋钮、以及这里的经验曲线**闭合成一条线**:$\lambda$ 不是玄学超参,它就是在这条偏差↔方差轴上找那个让总误差最小的点。